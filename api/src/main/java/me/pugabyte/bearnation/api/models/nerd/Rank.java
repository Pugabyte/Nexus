...
@AllArgsConstructor
public enum Rank implements IteratableEnum {
	...
	public List<Nerd> getOnlineNerds() {
		return Bukkit.getOnlinePlayers().stream()
				...
				.map(player -> (Nerd) new NerdService(BNCore.inst()).get(player))
	...
	public static Rank getHighestRank(OfflinePlayer player) {
		...
		for (Rank rank : ranks)
			if (BNCore.inst().getPerms().playerInGroup(null, player, rank.name()))
				return rank;
