package me.pugabyte.bearnation.api.models.geoip;
...
public class GeoIPService extends MongoService {
	...
	public GeoIPService(Plugin plugin) {
		super(plugin);
	}
	...
	private final String KEY = BNCore.config().getString("tokens.ipstack");
	...
	static {
		BNCore.addConfigDefault("tokens.ipstack", "abcdef");
	}
	...
	public GeoIP request(OfflinePlayer player, String ip) {
		if (ignore.contains(player.getUniqueId().toString()))
			return null;
		plugin.getLogger().info("Requesting GeoIP info for " + player.getName() + " (" + ip + ")");
