package me.pugabyte.bearnation.minigames.features.models.mechanics.multiplayer.teamless;
...
import me.pugabyte.bearnation.minigames.features.models.mechanics.multiplayer.MultiplayerMechanic;
...
public abstract class TeamlessMechanic extends MultiplayerMechanic {
	...
	public boolean shouldBeOver(Match match) {
		if (match.getAliveMinigamers().size() <= 1) {
			Minigames.log("Match has only one player, ending");
			return true;
		}
		...
		if (winningScore > 0)
			for (Minigamer minigamer : match.getAliveMinigamers())
				if (minigamer.getScore() >= winningScore) {
					Minigames.log("Teamless match has reached calculated winning score (" + winningScore + "), ending");
					return true;
				}
