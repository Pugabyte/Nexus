...
public class LoadoutMenu extends MenuUtils implements InventoryProvider {
	...
	public void init(Player player, InventoryContents contents) {
		contents.set(0, 0, ClickableItem.from(nameItem(
			...
			e -> {
				save(player);
				Minigames.getMenus().getTeamMenus().openTeamsEditorMenu(player, arena, team);
			}));
		...
		contents.set(0, 4, ClickableItem.from(
			new ItemBuilder(Material.POTION)
					...
					.build(),
			e -> Minigames.getMenus().getTeamMenus().openPotionEffectsMenu(player, arena, team)));
		...
		contents.set(0, 8, ClickableItem.from(nameItem(
			...
			e -> Minigames.getMenus().getTeamMenus().openDeleteLoadoutMenu(player, arena, team)));
