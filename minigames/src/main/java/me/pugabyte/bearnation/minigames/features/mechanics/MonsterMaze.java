...
import me.pugabyte.bearnation.minigames.features.models.matchdata.MonsterMazeMatchData;
import me.pugabyte.bearnation.minigames.features.models.mechanics.multiplayer.teamless.TeamlessMechanic;
import me.pugabyte.bearnation.minigames.features.utils.PowerUpUtils;
...
public class MonsterMaze extends TeamlessMechanic {
	...
	public void onPlayerJump(PlayerStatisticIncrementEvent event) {
		...
		if (item == null) {
			BNPlugin.warn("Player was allowed to jump without powerup");
			preventJump(minigamer);
			return;
		}

		if (item.getType() != Material.FEATHER) {
			BNPlugin.warn("Player was allowed to jump without powerup (Material is " + item.getType() + ")");
			preventJump(minigamer);
			return;
		}
