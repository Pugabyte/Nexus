...
public class PodiumsCommand extends CustomCommand {
	...
	static {
		Arrays.asList(Position.values()).forEach(position ->
				BNCore.addConfigDefault("minigames.podiums." + position.name().toLowerCase(), 0));
	}
	...
	private static int getPodiumId(Position position) {
		return BNCore.config().getInt("minigames.podiums." + position.name().toLowerCase());
	}

	@SneakyThrows
	private static void setPodiumId(Position position, int id) {
		BNCore.inst().getConfig().set("minigames.podiums." + position.name().toLowerCase(), id);
		BNCore.inst().saveConfig();
	}
