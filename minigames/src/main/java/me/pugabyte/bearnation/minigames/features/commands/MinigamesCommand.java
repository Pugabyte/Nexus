...
import me.pugabyte.bearnation.api.utils.Utils.RelativeLocation;
...
public class MinigamesCommand extends CustomCommand {
	...
	void acceptInvite() {
		...
		if (player().getWorld() != Minigames.getWorld()) {
			new WarpService(Minigames.inst()).get("minigames", WarpType.NORMAL).teleport(player());
			Minigames.tasks().wait(5, this::acceptInvite);
		} else
