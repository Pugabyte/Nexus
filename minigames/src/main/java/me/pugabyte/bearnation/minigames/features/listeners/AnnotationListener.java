...
import me.pugabyte.bearnation.minigames.features.mechanics.common.AntiCampingTask;
...
import me.pugabyte.bearnation.minigames.features.models.annotations.AntiCamp;
...
import me.pugabyte.bearnation.minigames.features.utils.Gun;
...
public class AnnotationListener implements Listener {
	...
	private void regenerate(Match match, String type) {
		...
		match.getArena().getWGUtils().getRegionsLike(regex).forEach(region -> {
			...
			match.getWEUtils().paster().file(file.toLowerCase()).at(region.getMinimumPoint()).paste();
		});
	}
