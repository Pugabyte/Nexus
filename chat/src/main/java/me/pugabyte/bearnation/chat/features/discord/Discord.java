...
import me.pugabyte.bearnation.api.BNAPI.Env;
...
public class Discord {
	...
	public Discord() {
		if (BNAPI.getEnv() != Env.PROD)
			return;
		Chat.tasks().repeatAsync(0, Time.MINUTE, this::connect);
		Chat.tasks().waitAsync(Time.SECOND.x(2), this::connect);
		Chat.inst().getCron().schedule("*/6 * * * *", Discord::updateTopics);
	}
		...
		for (Bot bot : Bot.values())
			if (bot.jda() == null && bot.getToken().length() > 0)
				try {
					...
					Chat.severe("An error occurred while trying to connect to Discord");
					ex.printStackTrace();
				}
	}
	...
	public static void addRole(String userId, DiscordId.Role role) {
		try {
			...
			if (roleById == null)
				Chat.log("Role from " + role.name() + " not found");
			else
