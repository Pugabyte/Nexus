...
import me.pugabyte.bearnation.chat.features.bridge.DiscordBridgeListener;
...
public enum Bot {
	...
	void connect() {
		if (this.jda == null && !isNullOrEmpty(getToken())) {
			final JDA jda = build();
			if (jda == null) {
				Chat.log("Could not connect " + name() + " to Discord");
				return;
			}
	...
	protected String getToken() {
		return BNCore.config().getString("tokens.discord." + name().toLowerCase(), "");
	}
