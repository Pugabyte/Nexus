...
public class Koda {
	...
	@Getter
	private static final Chatter chatter = new ChatService(Chat.inst()).get(player);

	public static void reply(String message) {
		Chat.tasks().wait(10, () -> say(message));
	}

	public static void replyIngame(String message) {
		Chat.tasks().wait(10, () -> sayIngame(message));
	}

	public static void replyDiscord(String message) {
		Chat.tasks().wait(10, () -> sayDiscord(message));
	}
		...
		if (config != null) {
			for (String key : config.getKeys(false)) {
				...
				if (!config.isConfigurationSection(key) || section == null)
					Chat.warn(PREFIX + "Configuration section " + key + " misconfigured");
				else
	...
	public static void respond(ChatEvent event, String response) {
		Chat.tasks().waitAsync(Time.SECOND, () -> {
			...
			case "griefing":
				if (event.getChatter() != null)
					if (!(event.getMessage().contains("not allowed") || event.getMessage().contains("isn't") || event.getMessage().contains("isnt")))
						if (Chat.inst().getPerms().playerHas(null, event.getChatter().getOfflinePlayer(), "rank.guest"))
