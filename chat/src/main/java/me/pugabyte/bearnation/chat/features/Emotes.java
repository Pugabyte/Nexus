...
import me.pugabyte.bearnation.chat.models.emote.EmoteService;
import me.pugabyte.bearnation.chat.models.emote.EmoteUser;
...
public enum Emotes {
	...
	public static void process(ChatEvent event) {
		...
		if (!Chat.inst().getPerms().playerHas(null, player, "emoticons.use"))
			return;
		if (!((EmoteUser) new EmoteService(Chat.inst()).get(player)).isEnabled())
			return;
