package me.pugabyte.bearnation.chat;
...
import me.pugabyte.bearnation.chat.features.ChatListener;
import me.pugabyte.bearnation.chat.features.ChatManager;
import me.pugabyte.bearnation.chat.features.alerts.AlertsListener;
import me.pugabyte.bearnation.chat.features.bridge.IngameBridgeListener;
import me.pugabyte.bearnation.chat.features.discord.DiscordId.Channel;
import me.pugabyte.bearnation.chat.features.translator.Translator;
...
public class Chat extends BNAPI {
	private static Chat staticInstance;
	...
	public void onEnable() {
		super.onEnable();
		new Timer("    Translator", () -> registerListener(new Translator()));
		new Timer("    ChatListener", () -> registerListener(new ChatListener()));
		new Timer("    IngameBridgeListener", () -> registerListener(new IngameBridgeListener()));
		new Timer("    AlertsListener", () -> registerListener(new AlertsListener()));
		new Timer("    addChannels", this::addChannels);
	}

	@Override
	public void onDisable() {
		new HashMap<>(new ChatService(Chat.inst()).getCache()).forEach((uuid, chatter) -> new ChatService(Chat.inst()).saveSync(chatter));
		super.onDisable();
	}

	private void updateChannels() {
		Bukkit.getOnlinePlayers().stream()
				.map(player -> (Chatter) new ChatService(Chat.inst()).get(player))
	...
	public static int getLocalRadius() {
		return BNCore.config().getInt("localRadius");
	}
