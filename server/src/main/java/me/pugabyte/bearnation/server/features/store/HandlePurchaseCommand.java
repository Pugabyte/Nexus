package me.pugabyte.bearnation.server.features.store;
...
import me.pugabyte.bearnation.server.models.purchase.Purchase;
import me.pugabyte.bearnation.server.models.purchase.PurchaseService;
...
import static me.pugabyte.bearnation.api.utils.StringUtils.uuidFormat;
...
public class HandlePurchaseCommand extends CustomCommand {
	...
	static {
		Tasks.repeatAsync(Time.SECOND, Time.MINUTE, () -> {
			...
			tasks.forEach(task -> {
				...
				if (packageType == null) {
					BNPlugin.severe("Tried to expire a package that doesn't exist: UUID: " + uuid + ", PackageId: " + packageId);
					return;
				}
	...
	void run(String data) {
		BNPlugin.log("Purchase caught; processing... " + data);
