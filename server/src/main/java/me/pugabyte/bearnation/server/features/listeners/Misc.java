...
import me.pugabyte.bearnation.features.minigames.managers.PlayerManager;
import me.pugabyte.bearnation.features.warps.Warps;
...
public class Misc implements Listener {
	...
	public void onJoin(PlayerJoinEvent event) {
		if (toSpawn.contains(event.getPlayer().getUniqueId())) {
			...
			BNPlugin.log("Teleporting resource world player " + event.getPlayer().getName() + " to spawn");
		...
		Tasks.wait(5, () -> {
			if (toSpawn.contains(event.getPlayer().getUniqueId())) {
				new WarpService().get("spawn", WarpType.NORMAL).teleport(event.getPlayer());
				BNPlugin.log("Teleporting resource world player " + event.getPlayer().getName() + " to spawn [2]");
				toSpawn.remove(event.getPlayer().getUniqueId());
			}
