...
import me.pugabyte.bearnation.BNCore.Env;
...
import me.pugabyte.bearnation.features.votes.EndOfMonth.TopVoterData;
...
//		},
		...
		Leaderboard(int... ids) {
			this.ids = ids;
			if (ids.length != 3)
				BNPlugin.warn(name() + " did not define 3 NPC ids (" + ids.length + ")");
		}
		...
		public void update() {
			Tasks.async(() -> {
				...
				if (top.size() != 3) {
					BNPlugin.warn(name() + " leaderboard top query did not return 3 results (" + top.size() + ")");
					return;
				}
