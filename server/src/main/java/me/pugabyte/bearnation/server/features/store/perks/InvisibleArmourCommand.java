...
import me.pugabyte.bearnation.server.models.invisiblearmour.InvisibleArmour;
import me.pugabyte.bearnation.server.models.invisiblearmour.InvisibleArmourService;
	...
	private static void sendPackets(InvisibleArmour invisibleArmour, ItemSlot slot) {
		...
		Bukkit.getOnlinePlayers().stream().filter(_player -> player.getWorld() == _player.getWorld()).forEach(_player -> {
			...
			try {
				...
				BNPlugin.log("Error trying to send invisible armour packets from " + player.getName() + " to " + _player.getName());
				ex.printStackTrace();
			}
		});
	}
