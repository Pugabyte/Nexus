...
import me.pugabyte.bearnation.server.models.afk.AFKService;
...
public class AFK {
	static Map<UUID, AFKPlayer> players = new AFKService(BNCore.inst()).getMap();

	public static void shutdown() {
		new AFKService(BNCore.inst()).saveAll();
	}

	static {
		BNCore.tasks().repeat(Time.SECOND.x(5), Time.SECOND.x(3), () -> Bukkit.getOnlinePlayers().stream().map(AFK::get).forEach(player -> {
			try {
